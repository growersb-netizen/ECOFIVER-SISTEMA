...
def ficha_modulo(m2: int) -> dict:
    ...
    titulo = f"Modulo habitacional prefabricado {m2}m2 celulosa {dormitorios}"[:60]
    descripcion = (
        f"INSTALACION EN EL DIA - GARANTIA 10 ANOS - CERTIFICADO DE CALIDAD PREMIUM\n\n"
        f"EcoFiver fabrica, transporta e instala todo con equipo propio. El precio incluye "
        f"la instalacion profesional completa. Al finalizar la jornada, el modulo queda "
        f"instalado y en pleno funcionamiento.\n\n"
        ...
        f"con escalera de acceso incluida. Si ya tenes platea de cemento, se apoya "
        f"directamente sobre ella.\n\n"
        f"Con {m2} m² de superficie, ideal para configurar como {dormitorios}, "
        f"oficina, local comercial, ampliacion de vivienda existente, espacio de trabajo, "
        f"sala de juegos, dormitorio de servicio o estudio en fondo de lote. "
        f"Nota importante: los modulos de 6 a 18 m² son espacios habitacionales complementarios, "
        f"no viviendas independientes.\n\n"
        f"Modalidad de pago: se entrega y se paga en el domicilio (pago contra entrega). "
        f"Cobertura: toda la Provincia de Buenos Aires. "
        f"Financiacion propia en cuotas disponible (sin banco ni tarjeta).\n\n"
        f"Garantia de 10 anos con certificado de calidad premium incluido. "
        f"Fabricacion propia en Zarate, Buenos Aires. Sin intermediarios."
    )
